...
using HuiAdminNetInfrastructure.Database;
using HuiAdminNetInfrastructure.IRepositoies;
using Microsoft.EntityFrameworkCore;
using MySqlConnector;
using System.Linq.Expressions;
using System.Reflection;

namespace HuiAdminNetInfrastructure.Repositoies
{
    public class BaseRepository : IBase
    {
        ...
        public async Task<List<T>> QueryAsync<T>(Expression<Func<T, bool>> whereLambda, bool isTrack = true) where T : class
        {
            if (isTrack)
            {
                return await Entities<T>().Where(whereLambda).ToListAsync<T>();
            }
            else
            {
                return await Entities<T>().Where(whereLambda).AsNoTracking().ToListAsync<T>();
            }
        }
        ...
        public async Task<List<T>> QueryAsync<T>(Expression<Func<T, bool>> whereLambda, bool isTrack = true, string orderBy = "") where T : class
        {
            var query = Entities<T>().Where(whereLambda);
        ...
        {
            List<T> listModifies = Entities<T>().Where(whereLamda).ToList();
            Type t = typeof(T);
